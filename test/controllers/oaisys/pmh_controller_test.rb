...
class Oaisys::PMHControllerTest < ActionDispatch::IntegrationTest
  ...
  def test_list_metadata_formats_route
    assert_recognizes({ controller: 'oaisys/pmh', action: 'list_metadata_formats' },
                      path: '?verb=ListMetadataFormats', method: :post)
    assert_recognizes({ controller: 'oaisys/pmh', action: 'list_metadata_formats' },
                      path: '?verb=ListMetadataFormats', method: :get)
  end
  ...
  def test_list_identifiers_route
    assert_recognizes({ controller: 'oaisys/pmh', action: 'list_identifiers' },
                      path: '?verb=ListIdentifiers', method: :post)
    assert_recognizes({ controller: 'oaisys/pmh', action: 'list_identifiers' },
                      path: '?verb=ListIdentifiers', method: :get)
  end
